...
`@sample_code`
```{r}
# La "c" en `c()` es la abreviatura de "concatenater." Concatenar es la acción de conectar elementos en una cadena.
# La función `c()` conecta todas las cadenas dentro de ella en un solo vector que podemos asignar a `x`
# Aquí, simplemente estamos usando `c()` para generar un vector al que podemos aplicar la función de tabla llamada `table()`
x <- c("a", "a", "b", "b", "b", "c")
# Aquí hay un ejemplo de lo que hace la función de tabla llamada 'table()'
table(x)
...
```{r}
test_error()
test_function("table",index=2, incorrect_msg = "Asegúrese de extraer la región variable del conjunto de datos.\.",not_called_msg = "Guarde el código de muestra.")
test_output_contains("table(murders$region)",incorrect_msg = "Use table en la variable murders$region.")
success_msg("¡Lo tienes! ¡Ahora eres un profesional creando tablas en R!")
```
