...
- Entonces use los operadores lógicos para crear un vector lógic y nómbralo `low`. Este vector nos dirá cual de las entradas de `murder_rate` son menores que 1, y cuales no lo son, en una sola línea de código.
...
```{r}
test_error()
test_object("low", undefined_msg = "Define low!", incorrect_msg = "Debe referirse a las tasas de homicidio menores que 1.")
...
`@instrucciones`
- Use los resultados del ejercicio anterior y la función `which` para determinar los índices `murder_rate` asociados con los valores menores que 1.
